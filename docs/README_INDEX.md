...
- 背包和角色页现在会显示装备机制说明卡，补上基础装备简介与词缀说明，并解释来源区域和词条来源
...
- 主要玩家页面的文案已经收成玩家视角，不再直接暴露测试进度或界面改版说明
- 背包现在支持背包内直接卸下装备、一键分解所有未锁定装备（含确认弹窗），以及装备锁定保护
- 锻造改为独立概率表（90%普通/8%稀有/1.5%史诗/0.5%传说）+5%附加词条升品
- 锻造、合成、分解操作全部增加游戏内确认弹窗，防止误触
- 全服事件新增AI风味文案（稀有掉落、世界Boss终结），由"墨丸"在村口播报
- 每日任务日志修复为中文显示，不再显示英文
