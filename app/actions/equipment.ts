  ...
  const result = await db.$transaction(async (tx) => {
    ...
    if (
      ...
      (!Number.isInteger(requestedSlotIndex) ||
        ...
        requestedSlotIndex >= getSlotCapacity(targetItem.slot as ItemSlot))
    ) {
      return { ok: false as const, reason: "invalid_slot_index" };
    }

    const targetSlot = targetItem.slot as ItemSlot;
    const targetSlotIndex =
      targetSlot === "accessory"
        ? requestedSlotIndex ??
          getFirstFreeEquipSlotIndex(toEquipmentSnapshot(items), targetSlot) ??
          0
        : 0;
    ...
    const nextItems = items.map((item) => {
      ...
      if (targetSlot !== "accessory" && item.slot === targetItem.slot) {
  ...
  await db.$transaction(async (tx) => {
    ...
    const selectedAssignments = itemSlots.flatMap((slot) =>
      (bestItems.get(slot) ?? []).map((item, index) => ({
        id: item.id,
        equipSlotIndex: slot === "accessory" ? index : 0,
      })),
    );
