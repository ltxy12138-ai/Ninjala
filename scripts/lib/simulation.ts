...
import { calculateBossWinChance } from "../../lib/game/boss";
import {
  ...
  for (const slot of itemSlots) {
    ...
    for (const [index, bestItem] of bestSlotItems.entries()) {
      ...
      if (playerItem) {
        ...
        playerItem.equipSlotIndex = slot === "accessory" ? index : 0;
      }
    }
  }
...
function attemptBossProgress(
  ...
  while (safety < 20) {
    ...
    if (random() >= calculateBossWinChance(player.power, boss.power)) {
      continue;
    }
    ...
    const droppedItems = Array.from({ length: boss.rewardItemCount }, () =>
      generateEquipmentDrop(boss.rewardDropTableId, random, locale),
    );
...
function buildSnapshot(players: SimPlayer[], day: number): SimulationSnapshot {
  ...
  return {
    ...
    finalRegionPlayers: highestRegions.region_005 ?? 0,
    highestRegions,
  };
}
