...
describe("boss challenges", () => {
  ...
  it("unlocks the next region after a successful clear", async () => {
    ...
    expect(result.battleSummary.rewardSummary.items.length).toBeGreaterThanOrEqual(2);
    expect(
      result.battleSummary.rewardSummary.items.some((itemName) => itemName.length > 0),
    ).toBe(true);
