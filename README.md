...
- `npm run sim:balance` - inspect late-game best-set ceilings against region gates and bosses
...
- inventory, 7-slot gear equip, dual ring handling, one-click best equip, and power recalculation
- a 10-region main route with longer progression gates, bosses, rankings, and logs
- late-game main-route gates, boss power, and region 6-10 gear curves have been rebalanced together so displayed power can keep climbing into the back half of the route
...
- displayed power now counts a light share of `crit`, while reward-focused stats still stay out of the gate formula
- gear now follows a more TQ-style structure with regional gear families, themed affix pools, and first-clear boss reward spikes
- gear detail cards now keep affix sections minimal by showing only stat lines and values instead of explanatory prose
...
- daily tasks with progress tracking and reward claiming on the home page, driven by existing game events in `GameLog`
- friend activity feed on the home page showing recent global events (boss clears, region unlocks, rare drops, blessings)
...
- closed-test feedback on whether current progression pacing still needs another post-V1 late-game pass

## Implemented Routes

- `/home` - overview, daily tasks, route shortcuts, and project-status tabs
...
- `/characters` - separate stats and gear tabs with a slot-selection equipment panel across weapon, helmet, armor, boots, bracer, amulet, and two ring positions
